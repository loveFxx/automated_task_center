...
import com.sailvan.dispatchcenter.stat.monitor.statistics.ContinentAccountStat;
import com.sailvan.dispatchcenter.stat.monitor.statistics.MachineHeartBeatLogStat;
import com.sailvan.dispatchcenter.stat.monitor.statistics.MachineStat;
import com.sailvan.dispatchcenter.stat.monitor.statistics.TaskLogStat;
import com.sailvan.dispatchcenter.stat.monitor.constant.MonitorConstant;
import com.sailvan.dispatchcenter.stat.monitor.util.HttpDownloadUtils;
...
public class DownloadController {
     ...
     * @return
     */
    @RequestMapping(value = "/downloadSiteExcelFromMonitor", method = RequestMethod.POST)
    ...
    public String downloadExcelFromMonitor(HttpServletResponse response, @PathVariable String excelType) {
        ...
        switch (excelType) {
            ...
            //亚马逊失效代理IP
            case "downloadAmazonInvalidIpExcel" : {
                List<ProxyIp> proxyIpAll = proxyIpService.getProxyIpAll();
                for (ProxyIp proxyIp : proxyIpAll) {
                    //在所有代理IP中找出失效的
                    if (proxyIp.getValidStatus() == 0){
                        List<StoreAccount> storeAccountByIp = storeAccountDao.getStoreAccountByIp(proxyIp.getIp());
                        if (storeAccountByIp.size() != 0){
                            //筛选出亚马逊对应的代理IP 也就是找出有对应账号站点的代理IP
                            for (StoreAccount storeAccount:storeAccountByIp){
                                infoList.add(new String[]{storeAccount.getProxyIp(), String.valueOf(proxyIp.getPort()),storeAccount.getAccount(),storeAccount.getContinents() });
                            }
                        }
                ...
                stringArr = new String[]{ "代理ip","端口","账号","站点"};
                filePath = System.getProperty("user.dir") + "/stat" + "/amazonInvalidIp.xlsx";
                ExcelUtils.createExcelFile3(filePath, stringArr, infoList);
                break;
            }
