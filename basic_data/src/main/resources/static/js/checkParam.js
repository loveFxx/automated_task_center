layui.use('form', function(){
    ...
    form.verify({
        ...
        stringCheck: [/^.{1,50}$/, '太长了'],


        numCheck: [/^-?[1-9]\d*$/, '只能使用整数'],

        accountCheck: [/^[A-Za-z0-9_]{1,50}$/, '只能使用数字英文或者下划线'],

        tinyIntCheck: [/^(-128|\-?([0-9]|[1-9][0-9]|1[0-2][0-7]))$/, '只能使用-128~127的数字'],

        macCheck: [/^([A-Fa-f0-9]{2}:){5}[A-Fa-f0-9]{2}$/, '只能使用格式为FA:16:3E:4A:01:76的mac地址'],

        ipCheck: [/^((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3}$/, '只能使用ip地址'],
